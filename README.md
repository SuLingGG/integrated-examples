...
1. 实现了反代使用 Local Loopback 连接或使用 UDS（Unix Domain Socket） 连接。
2. 实现了回落/分流使用 Local Loopback 连接及启用 PROXY protocol 支持，或使用 UDS（Unix Domain Socket） 连接及启用 PROXY protocol 支持。
3. 实现了 SNI 分流使用 Local Loopback 连接及启用 PROXY protocol 支持，或使用 UDS（Unix Domain Socket） 连接及启用 PROXY protocol 支持。
4. 实现了 Caddy 与 V2Ray/Xray 的应用使用 UDS（Unix Domain Socket） 连接时采用 Abstract sockets 模式，不需考虑 UDS 文件路径及权限。
...
7. 实现了由 Caddy 提供 TLS 证书的应用，其证书申请、更新及重载更新都可全自动化。
8. 实现了正常应用与 CDN 流量中转（基于 WebSocket/HttpUpgrade/gRPC over TLS） 应用同时使用。
9. 实现了服务端综合应用配置示例中使用 mKCP、WebSocket、HTTP/2（H2/H2C）、gRPC 传输方式的应用配置可删、可换、可增（REALITY H2/gRPC 应用除外），灵活改变而不影响整体架构。
...
#### &emsp;Xray 的反代 HttpUpgrade 应用（WebSocket 应用的替代）
...
#### &emsp;由 Nginx 专职 SNI 分流实现负载均衡的综合应用
...
5. 目前 Caddy 从 Let's Encrypt 或 ZeroSSL 自动申请的 TLS 证书默认都为 ECC 证书。
6. 综合应用配置示例中使用 mKCP、WebSocket、HTTP/2（H2/H2C）、gRPC 传输方式的应用配置可删、可换、可增（REALITY H2/gRPC 应用除外）；参考‘服务端单一/简单应用配置示例’中对应配置示例修改。
7. 当前 WebSocket 传输方式的应用仅推荐使用 CDN 流量中转，其相关 Xray 的应用推荐使用 HttpUpgrade 传输方式替代，功能相同且效率更高。
8. 若 CDN 流量中转（基于 WebSocket/HttpUpgrade/gRPC over TLS）使用不可信的 CDN 进行中转，V2Ray/Xray 示例推荐使用自己带加密的 VMess 或 Shadowsocks 协议配置；否则推荐使用自己不带加密的 VLESS 或 Trojan 协议配置。
9. 针对 V2Ray 特性，当前服务端配置推荐使用非 AES 算法密码套件配置示例，客户端开启 uTLS 指纹。 
10. 针对 Xray 特性，当前服务端配置依次推荐使用 REALITY Vision 配置示例、XTLS Vision 配置示例、非 AES 算法的密码套件配置示例、REALITY 配置示例，客户端开启 uTLS 指纹。
11. 流量伪装与防探测网站可由其它 WEB 应用软件实现，其支持反代应用（反代 WebSocket、HttpUpgrade、gRPC 及 H2C）与支持回落应用（提供 H2C server 及 HTTP/1.1 server 的 WEB 服务）等取决于自身，其配置自行参考 Caddy 或 Nginx 对应配置示例。
12. 附加相关插件的 Caddy 程序文件已编译好，去本人 Releases 中下载即可。
13. Trojan-Go 安卓手机客户端可以去本人 Releases 中下载（最末）。
